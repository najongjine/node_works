...
소멸되는 method: 성능상 이슈를 발생시킬수 있는 소지가 있어
 소멸하거나 다른 method로 대치(변경)하기로 결정.
처음 컴포넌트를 생성하고 start 했을때
1. constructor()가 실행
...
class TodoList extends Component {
  ...
  /*
  화면에 리스트를 표시하기위한 todlList 배열이 변경이 되었느냐를 판단해서
  render()함수를 호출할지 안할지를 알려주는 method
  js에서 객체(배열)==객체(배열) 하면 간혹 비슷한 주소위치를 참조하여 서로 내용이 다름에도 같은 배열로 나타나는 경우가 있다.
  객체(배열)===객체(배열) 하면 깊이비교(내용) 비교를 하여 정확한 결과를 얻을수 있다.
  */
