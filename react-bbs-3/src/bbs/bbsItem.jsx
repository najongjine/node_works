...
/*
표현할 데이터가 List 형태일때 List의 한줄(한 요소)를 표현하는 구조로 생성
*/
...
//이럴땐 this.props.history 객체를 직접 전달받지 못한다.
// 1대 자손이 아닌 컴포넌트에서 this.props.history 객체를 사용하기 위해서는 객체를 withRouter() method로
//wrapping을 해줘야 한다.
...
핸들러 이벤트를 등록할때는 함수를 객체 타입으로 바꾸어서 등록을 한다
함수 이름에서 괄호를 반드시 제거하고 등록.
만약 이벤트 핸들러로 함수 타입 =함수() 으로 등록을 하게되면
각 컴포넌트가 랜더링 되는 동안 이벤트 핸들러로 등록 하는게 아니라 함수를 호출하여 실행해 버린다
...
이렇게 등록을 하면 랜더링 하는 동안에는 (e)=>{} 의 익명 이벤트만 바라보고
이벤트 핸들러로 등록
이때 내부에 작성된 코드는 모두 무시된다.
비로소 모든 랜더링이 끝나고 이벤트 실행을 하게되면
이벤트 핸들러가 자신이 포함하고 있는 함수를 호출하여 원하는 코드를 실행하게 된다.
*/
